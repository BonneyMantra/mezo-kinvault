...
contract DeployMezo {
    ...
    address private constant MEZO = 0x7B7c000000000000000000000000000000000001;
    uint256 private constant DEFAULT_HEARTBEAT_INTERVAL = 60;
    uint256 private constant DEFAULT_KEEPER_REWARD_BPS = 1000; // 10% of MEZO bond to the keeper
    ...
    function run() external returns (KinVault vault) {
        ...
        uint256 keeperRewardBps = vm.envOr("MEZO_KEEPER_REWARD_BPS", DEFAULT_KEEPER_REWARD_BPS);

        vm.startBroadcast(privateKey);
        vault = new KinVault(owner, BORROWER_OPS, PRICE_FEED, MUSD, MEZO, heartbeatInterval, uint16(keeperRewardBps));
        vm.stopBroadcast();
        ...
        vm.serializeAddress(root, "mezo", MEZO);
        vm.serializeUint(root, "heartbeatInterval", heartbeatInterval);
        vm.serializeUint(root, "keeperRewardBps", keeperRewardBps);
